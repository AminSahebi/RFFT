...
//float X_i[8] , X_r[8], temp_i[8], temp_r[8];	
DATA X_i[8] , X_r[8], temp_i[8], temp_r[8];	
int FFT_N = 8;
//#define DEBUG 
/*******************************************************************************
...
/*
   inline byte bit_reverse(byte c); __attribute__ ((always_inline))
...
{ 
...
#ifdef DEBUG
	printf("[INFO]: Re-ordering phase started...\n");
#endif
	...
	for(int i=0;i<n;i++)
	{
		...
		if(i>j){
			DATA tmp_r[sizeof(i)*8-log_n];  //re-ordering
			tmp_r[i] = my[i];
			my[i] = my[j];
			my[j] = tmp_r[i];
		}
	...
	memcpy(temp_r,my, 8 * sizeof(DATA));
...
#endif
	my = (DATA*)malloc (8*sizeof(DATA));
	out = (DATA*)malloc (8*sizeof(DATA));
	
	for (int i = 0; i < FFT_N; i++) 
	{
		my[i] = (DATA)(2.0*i)+ (DATA)1;	
	}
#ifdef DEBUG
...
void report(){
	...
	DATA Err = sum_input-(sum_output/FFT_N);
	logmsg("Using float Variables...");
	logmsg("the abs of inputs are %.9lf",sum_input);
	logmsg("the abs of out puts are %.9lf",sum_output/FFT_N);
	logmsg("The Err for is %.9lf",sum_input-(sum_output/FFT_N));
