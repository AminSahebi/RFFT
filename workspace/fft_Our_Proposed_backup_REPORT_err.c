...
void report(){
	...
	for (int i=0;i<FFT_N;i++)
	{
		sum_input+=sqrt(my[i]*my[i])*sqrt(my[i]*my[i]);
		sum_output+=(sqrt(X_r[i]*X_r[i] + X_i[i]*X_i[i]))*(sqrt(X_r[i]*X_r[i] + X_i[i]*X_i[i]));
	}
	printf("the abs of inputs are %f\n",sum_input);
	printf("the abs of out puts are %f\n",sum_output/FFT_N);
	printf("The Parseval's theorem proved the FFT procedure\n");
	printf("The Err is %.6lf\n",sqrt(sum_input-(sum_output/FFT_N)));
